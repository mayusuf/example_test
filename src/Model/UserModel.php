...
final class UserModel extends Model
{
	...
	public function save($model){		
		...
		$columns = " (firstName, lastName, email, uuid, pass, mobileNumber, address, city, zipCode, country, apiKey, createDate) ";
		...
		'".$this->escapeSQLinjection($model->address)."',
		'".$this->escapeSQLinjection($model->city)."',
		'".$this->escapeSQLinjection($model->zipCode)."',
	...
	public function login($model){	
		...
		$where = " email = '".$this->escapeSQLinjection($model->email)."' && pass =	'".$this->escapeSQLinjection($model->pass)."' && isActive = 1";
	...
	public function update($model){		
		...
		address = '".$this->escapeSQLinjection($model->address)."',
		city = '".$this->escapeSQLinjection($model->city)."',
		zipCode = '".$this->escapeSQLinjection($model->zipCode)."',
	...
	public function checkApiKey($apiKey,$userId=''){	
		
		if(!empty($userId)){

			$sql = "SELECT id From $this->table WHERE apiKey = '".$apiKey."' AND id = $userId AND isActive = 1";	
		}else{
	...
	public function findAll(){	
		...
		$this->sql = "Select * From $this->table WHERE isActive=1";
